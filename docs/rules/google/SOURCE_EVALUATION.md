...
4. Sukka 与 BlackMatrix7 已加入自动只读验证；完整规则不持久化，报告只
   保存已有证据的例外和多来源共同指出的未决最小集合，也不自动合并。
5. Google 主表构建保持独立；GoogleCN、GoogleAI 与 AI 分别由专用流水线生成，`@cn`、`@ads` 在 Google 主表中仍只作为元数据审计。
6. `reference-audit.json` 自动过滤已覆盖和单一来源差异；只对
   `verification.manual_review` 中的多来源未决项进行人工核对。
