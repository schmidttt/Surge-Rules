...
人工补丁不能绕过 Google、国内 AI 或两张产物互斥门禁。当前精确补丁
包括 JetBrains 官方网络文档确认的 `api.jetbrains.ai`、Google AI 入口
`g.ai` 和独立 AI 服务 `ai.com`，均不放大为其父域后缀。
...
- `g.ai` 与 `ai.com` 只加入经验证的精确主机；
- 其余单一参考差异由程序记录但不改变产物，多个参考源共同指出的未决
  差异才进入人工审核。
