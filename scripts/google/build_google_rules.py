...
def render_rules(title: str, source: SourceMetadata, rules: Sequence[Rule]) -> str:
    header = [
        "# {}".format(title),
        "# Source: https://github.com/{}".format(source.repository),
        "# Source commit: {}".format(source.commit),
        "# Data license: MIT; Copyright (c) 2018-2019 V2Ray.",
        "# License notice: THIRD_PARTY_LICENSES/v2fly-MIT.txt",
        "# Generated by scripts/google/build_google_rules.py; review before use.",
...
def validate_outputs(
    ...
    if existing_main:
        ...
        ratio = abs(len(new) - len(existing_main)) / max(len(existing_main), 1)
        if ratio > max_change_ratio:
            raise BuildError(
                "Google.list size changed by {:.1%}, above {:.1%}; review and rerun "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    files = {
        Path("rules/Google/Google.list"): render_rules(
            "Remaining Google ecosystem for Surge", v2fly_meta, main_rules
        ),
