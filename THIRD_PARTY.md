...
- 许可证：MIT。
- 用途：唯一正式规则数据上游。
- 使用方式：读取固定提交中的 `data` 目录，按其 include、属性和 affiliation 语义生成 Surge `Google.list`。
...
- 用途：构建时在线读取 Google 规则，只计算双方总量、交集数量、差异数量和非域名规则类型数量。
...
- 用途：构建时在线读取固定提交中的 `GLOBAL.GOOGLE` 和 `ai.conf` Google 段，仅进行覆盖分类统计。
...
- 正式 `Google.list` 只由 v2fly 数据和本项目显式补丁生成。
